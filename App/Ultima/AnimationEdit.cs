...
namespace UltimaSDK
{
	...
	public sealed class AnimIdx
	{
		...
		public AnimIdx(int index, FileIndex fileIndex, int filetype)
		{
			...
			int length, extra;
			Stream stream = fileIndex.Seek(index, out length, out extra, out _);
	...
	//Soulblighter Modification
		...
		public unsafe void ExportPalette(string filename, int type)
		{
			switch (type)
			{
				...
				case 1:
					{
						var bmp = new Bitmap(0x100, 20, Config.PIXEL_FORMAT);
						var bd = bmp.LockBits(new Rectangle(0, 0, 0x100, 20), ImageLockMode.WriteOnly, Config.PIXEL_FORMAT);
						var line = (ushort*)bd.Scan0;
						var delta = bd.Stride >> 1;
	...
	public sealed class FrameEdit
	{
		...
		public unsafe FrameEdit(Bitmap bit, ushort[] palette, int centerx, int centery)
		{
			...
			width = bit.Width;
			height = bit.Height;
			var bd = bit.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, Config.PIXEL_FORMAT);
			var line = (ushort*)bd.Scan0;
			var delta = bd.Stride >> 1;
			var tmp = new List<Raw>();
